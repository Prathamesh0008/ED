...
export default function LoginPopup({ isOpen, onClose, onLoginSuccess }) {
  ...
  const [addrDetails, setAddrDetails] = useState({ street: "", city: "", pincode: "" });
  ...
  useEffect(() => {
    if (isOpen) {
      setIsVisible(true);
      setMessage(""); 
      setFormData({ username: "", email: "", gender: "", mobile: "", password: "", confirmPassword: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isLogin && formData.password !== formData.confirmPassword) {
      setMessage(T.errors.passwordMismatch); // TRANSLATED
      return;
    }
    ...
    setMessage(T.messages.processing); // TRANSLATED
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        setMessage(data.message || T.errors.operationFailed); // TRANSLATED Fallback
        return;
      }
      ...
      // ✅ SIGNUP SUCCESS
        ...
        setMessage(T.messages.accountCreated); // TRANSLATED
        setIsLogin(true);
      }
    } catch (err) {
      if (err.name === "AbortError") return;
      setMessage(T.errors.serverError); // TRANSLATED
    }
  };

  // Shared Input Styles for Consistency (Square look)
  const inputStyle = "w-full px-4 py-3 bg-gray-50/50 border border-gray-200 focus:bg-white text-gray-700 text-sm outline-none focus:border-[#2f609b] focus:ring-1 focus:ring-[#2f609b] transition-all rounded-none placeholder:text-gray-400";

  return (
    <div 
      className={`fixed inset-0 z-[100] flex justify-center items-center transition-all duration-500 ease-in-out px-4 py-4
      ${isOpen ? "bg-[#0f172a]/70 backdrop-blur-md opacity-100" : "bg-transparent opacity-0 pointer-events-none"}`}
    >
      <div 
       ...
       {/* Close Button */}
        ...
        <div className="text-center mb-8 mt-2">
            <h2 className="text-3xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-[#1d275e] to-[#2f609b]">
                {isLogin ? T.headers.welcome : T.headers.join}
            </h2>
            <p className="text-gray-400 text-xs font-medium mt-2 uppercase tracking-wide">
                {isLogin ? T.headers.dashboard : T.headers.create}
            </p>
        </div>
            ...
            <>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (value === "") {
                              ...
                              setCityError(T.errors.invalidInput); // TRANSLATED
                            } else {
                              setCityError("");
                            }
                      ...
                      <div className="group">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            // ❌ wrong input (letters / symbols)
                            if (value === "") {
                              setPincodeError("");
                            } else if (/[^0-9]/.test(value)) {
                              setPincodeError(T.errors.invalidInput); // TRANSLATED
                            } else {
                              setPincodeError("");
                            }
                        ...
                        {/* ❗ error EXACTLY below pincode box */}
                        {pincodeError && (
                          <p className="text-red-500 text-[11px] mt-1 font-medium">
                            {pincodeError}
                          </p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="group">
                  <input 
                    ...
                    onChange={(e) => {
                      ...
                      if (/[^0-9]/.test(value)) {
                        setMobileError(T.errors.invalidInput); // TRANSLATED
                      } else {
                        setMobileError("");
                      }
                ...
                <div className="group">
                  ...
                  >
                    <option value="" disabled>{T.labels.genderDefault}</option>
                    <option value="Male">{T.labels.genderMale}</option>
                    <option value="Female">{T.labels.genderFemale}</option>
                    <option value="Other">{T.labels.genderOther}</option>
                  </select>
                </div>
              </div>
            </>
        ...
        {message && (
          <div className={`mt-4 p-3 rounded-none text-xs font-bold text-center border-l-4 ${
