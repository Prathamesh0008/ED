...
export default function ProductsPage() {
  ...
  const { t } = useLanguage(); 
  // Safety check to prevent crash if t.productsPage is missing
  const T = t?.productsPage || {
    hero: { suffix: "Products", subtitle: "Browse through our comprehensive range of pharmaceutical products" },
    filters: { searchPlaceholder: "Search...", allCompounds: "All Compounds", resetButton: "Reset", clearSearch: "Clear" },
    productCard: { viewDetails: "View Details", addToCart: "Add to Cart", dosageLabel: "Dosage:", compositionLabel: "Composition:", packSizeLabel: "Pack Size:", skusAvailable: "SKUs", productsAvailable: "products available", productAvailable: "product available" },
    emptyState: { title: "No Products Found", description: "We couldn't find any products matching your search criteria." }
  };
        ...
        <div className="mb-8 sm:mb-12">
          <div className="text-center mb-6 sm:mb-10">
            <h1 className="text-3xl xs:text-4xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-2 sm:mb-4">
              ...
              <span className="text-2xl xs:text-2xl sm:text-5xl block sm:inline">{T.hero.suffix}</span>
            </h1>
    ...
    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
      ...
      <div className="relative flex-1">
        <select
          ...
          style={{ '--tw-ring-color': theme.primary }}
        >
          <option value="">{T.filters.allCompounds}</option>
          {compoundNames.map((c) => (
        ...
        <div ref={productsStartRef} className="pt-4 sm:pt-8">
                  ...
                  <div className="flex flex-col xs:flex-row xs:items-center justify-between gap-3 xs:gap-0 mb-4 sm:mb-6">
                    <div>
                      ...
                      <p className="text-gray-600 text-sm sm:text-base mt-1 sm:mt-2">
                        {items.length} {items.length !== 1 ? T.productCard.productsAvailable : T.productCard.productAvailable}
                      </p>
                    </div>
                    <div className="px-2 sm:px-4 py-1 sm:py-2 rounded-full text-xs sm:text-sm font-semibold"
                         style={{ backgroundColor: `${theme.primary}15`, color: theme.primary }}>
                      {slugs.length} {T.productCard.skusAvailable}
                    </div>
                  </div>
                                ...
                                <div className="space-y-2">
                                  {p.dosage && (
                                    <div className="flex items-start">
                                      <span className="text-xs sm:text-sm font-medium text-gray-700 min-w-[70px] sm:min-w-[80px]">{T.productCard.dosageLabel}</span>
                                      <span className="text-xs sm:text-sm text-gray-600 ml-2">{p.dosage}</span>
                                    </div>
                                  )}
                                  {p.composition && (
                                    <div className="flex items-start">
                                      <span className="text-xs sm:text-sm font-medium text-gray-700 min-w-[70px] sm:min-w-[80px]">{T.productCard.compositionLabel}</span>
                                      <span className="text-xs sm:text-sm text-gray-600 ml-2">{p.composition}</span>
                                    </div>
                                  )}
                                  {p.pack_size && (
                                    <div className="flex items-start">
                                      <span className="text-xs sm:text-sm font-medium text-gray-700 min-w-[70px] sm:min-w-[80px]">{T.productCard.packSizeLabel}</span>
                                    ...
                                    <div className="space-y-2 sm:space-y-3">
                                      {p.dosage && (
                                        <div className="flex items-center">
                                          <span className="text-sm font-medium text-gray-700 min-w-[90px] sm:min-w-[100px]">{T.productCard.dosageLabel}</span>
                                          <span className="text-sm text-gray-600 ml-2 sm:ml-3">{p.dosage}</span>
                                        </div>
                                      )}
                                      {p.composition && (
                                        <div className="flex items-center">
                                          <span className="text-sm font-medium text-gray-700 min-w-[90px] sm:min-w-[100px]">{T.productCard.compositionLabel}</span>
                                          <span className="text-sm text-gray-600 ml-2 sm:ml-3">{p.composition}</span>
                                        </div>
                                      )}
                                      {p.pack_size && (
                                        <div className="flex items-center">
                                          <span className="text-sm font-medium text-gray-700 min-w-[90px] sm:min-w-[100px]">{T.productCard.packSizeLabel}</span>
          ...
          {compoundNames.length > 0 && !hasAnyResults && (
            <div className="text-center py-12 sm:py-20 px-4">
              ...
              <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2 sm:mb-3">{T.emptyState.title}</h3>
